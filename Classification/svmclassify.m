function [outclass, decDist] = svmclassify(svmStruct,sample, varargin)
%[EDITED built in function to output decision boundary distance]
%SVMCLASSIFY Classify data using a support vector machine
%   [GROUP, decisionBoundaryDistance] = SVMCLASSIFY(SVMSTRUCT, TEST) classifies each row in TEST using
...
%   TEST is assigned.
%
%   [GROUP, decisionBoundaryDistance] = SVMCLASSIFY(...,'SHOWPLOT',true) plots the test data TEST on
...
%   $Revision: 1.1.12.13 $  $Date: 2012/05/03 23:56:55 $
...
if ~isempty(sample)
    ...
    try
        [outclass,decDist] = svmdecision(sample,svmStruct);
    catch ME
...
%   Copyright 2004-2012 The MathWorks, Inc.
%   $Revision: 1.1.12.6 $  $Date: 2012/05/03 23:57:05 $
