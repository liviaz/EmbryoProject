...
%allow the user to specify where to load the settings file
[filename, pathname] = uigetfile('C:\Users\Livia\Desktop\IVF\Raw Data\Yi\InputFiles\*.*', 'Pick a file');
    if isequal(filename,0) | isequal(pathname,0)
       return
    end
...
%allow the user to specify where to load the settings file
dirname = uigetdir('C:\Users\Livia\Desktop\IVF\Raw Data\Yi\InputFiles', 'Pick a folder containing ascii file distributions');
...
for i=1:3
    if checked(i)
        ...
        legend('Set 1','Set 2','Set 3','Location','NorthWest');
        ...
        xlabel('t'),ylabel('K(t)'), title('All K-functions')
        ...
        xlabel('t'),ylabel('K(t) - E(K(t))'), title('All K-functions')
...
for i=i
    for j=j
        if i~=j
            boots_12=kboot({handles.set_K_array{i} handles.set_K_array{j}},{handles.set_nbrofevents{i} handles.set_nbrofevents{j}},{handles.set_residuals{i} handles.set_residuals{j}},5000,'between');
            result=btss({handles.set_K_array{i} handles.set_K_array{j}},{handles.set_nbrofevents{i} handles.set_nbrofevents{j}},t,boots_12);
            p=result.p;
            BTSS=result.BTSS;
            ...
            title(['Between Group comparations', 10, 'Set ' num2str(i) ' and ' num2str(j), 10, 'P-value =' num2str(p)]);
...
csr_boots=kboot(CSR_K_array,csr_nbr_of_events, csr_res, 1000,'within');
...
boots=kboot({K_array CSR_K_array},{nbr_of_events csr_nbr_of_events},{res res_csr},5000,'between');
